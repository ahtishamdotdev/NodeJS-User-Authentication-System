...
* updated well known delivery service list ([fa2724b](https:
...
* **pools:** Emit 'clear' once transporter is idle and all connections are closed ([839e286](https:
* **smtp-connection:** jsdoc public annotation for socket ([#1741](https:
* **well-known-services:** Added AliyunQiye ([bb9e6da](https:
...
* **attachments:** Set the default transfer encoding for message/rfc822 attachments as '7bit' ([007d5f3](https:
...
* **ses:** Fixed structured from header ([faa9a5e](https:
...
* **ses:** Use formatted FromEmailAddress for SES emails ([821cd09](https:
...
* SESv2 SDK support, removed older SES SDK v2 and v3 , removed SES rate limiting and idling features ([15db667](https:
...
* close correct socket ([a18062c](https:
...
* **services:** add Seznam email service configuration ([#1695](https:


### Bug Fixes

* **proxy:** Set error and timeout errors for proxied sockets ([aa0c99c](https:
...
* **addressparser:** Correctly detect if user local part is attached to domain part ([f2096c5](https:
...
* Fix memory leak ([#1667](https:
* **mime:** Added GeoJSON closes [#1637](https:
...
* **api:** Added support for Ethereal authentication ([56b2205](https:
* **services.json:** Add Email Services Provider Feishu Mail (CN) ([#1648](https:
* **services.json:** update Mailtrap host and port in well known ([#1652](https:
* **well-known-services:** Add Loopia in well known services ([#1655](https:
...
* **tls:** Ensure servername for SMTP ([d66fdd3](https:
...
* **message-generation:** Escape single quote in address names ([4ae5fad](https:
...
* **headers:** Ensure that Content-type is the bottom header ([c7cf97e](https:
...
* **data-uri:** Do not use regular expressions for parsing data URI schemes ([12e65e9](https:
* **data-uri:** Moved all data-uri regexes to use the non-regex parseDataUri method ([edd5dfe](https:
...
* **security:** Fix issues described in GHSA-9h6g-pr28-7cqp. Do not use eternal matching pattern if only a few occurences are expected ([dd8f5e8](https:
* **tests:** Use native node test runner, added code coverage support, removed grunt ([#1604](https:
...
* **punycode:** do not use native punycode module ([b4d0e0c](https:
...
* **customAuth:** Do not require user and pass to be set for custom authentication schemes (fixes [#1584](https:
...
* **inline:** Use 'inline' as the default Content Dispostion value for embedded images ([db32c93](https:
* **tests:** Removed Node v12 from test matrix as it is not compatible with the test framework anymore ([7fe0a60](https:
...
* **license:** Updated license year ([da4744e](https:
...
-   Added new option to pass a set of httpHeaders to be sent when fetching attachments. See [PR #1034](https:
...
-   Added new methods `createTestAccount` and `getTestMessageUrl` to use autogenerated email accounts from https:
...
-   Added built-in transport for AWS SES. [Docs](http:
-   Updated stream transport to allow building JSON strings. [Docs](http:
...
-   **OAuth2 authentication** is built in and has a different [configuration](https:
-   **Improved Calendaring**. Provide an ical file to Nodemailer to send out [calendar events](https:
...
-   Implemented templating support. You can either use a simple built-in renderer or some external advanced renderer, eg. [node-email-templates](https:
...
Added new message option `watchHtml` to specify Apple Watch specific HTML part of the message. See [this post](https:
...
Total rewrite. See migration guide here: <http:
