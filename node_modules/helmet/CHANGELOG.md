...
- `Content-Security-Policy` gives a better error when a directive value, like `self`, should be quoted. See [#482](https:
...
- **Breaking:** `Content-Security-Policy` middleware now throws an error if a directive should have quotes but does not, such as `self` instead of `'self'`. See [#454](https:
...
- `Content-Security-Policy` middleware now warns if a directive should have quotes but does not, such as `self` instead of `'self'`. This will be an error in future versions. See [#454](https:

## 7.1.0 - 2023-11-07

### Added

- `helmet.crossOriginEmbedderPolicy` now supports the `unsafe-none` directive. See [#477](https:

## 7.0.0 - 2023-05-06

### Changed

- **Breaking:** `Cross-Origin-Embedder-Policy` middleware is now disabled by default. See [#411](https:

### Removed

- **Breaking:** Drop support for Node 14 and 15. Node 16+ is now required
- **Breaking:** `Expect-CT` is no longer part of Helmet. If you still need it, you can use the [`expect-ct` package](https:
...
- Fixed yet another issue with TypeScript exports. See [#420](https:

## 6.1.4 - 2023-04-10

### Fixed

- Fix another issue with TypeScript default exports. See [#418](https:

## 6.1.3 - 2023-04-10

### Fixed

- Fix issue with TypeScript default exports. See [#417](https:
...
- Improve support for various TypeScript setups, including "nodenext". See [#405](https:

## 6.0.1 - 2022-11-29

### Fixed

- `crossOriginEmbedderPolicy` did not accept options at the top level. See [#390](https:
...
- **Breaking:** `helmet.expectCt` is no longer set by default. It can, however, be explicitly enabled. It will be removed in Helmet 7. See [#310](https:
- **Breaking:** Increase TypeScript strictness around some arguments. Only affects TypeScript users, and may not require any code changes. See [#369](https:
...
- Fix TypeScript bug with some TypeScript configurations. See [#375](https:

## 5.1.0 - 2022-05-17

### Added

- `Cross-Origin-Embedder-Policy`: support `credentialless` policy. See [#365](https:
...
- Improve imports for CommonJS and ECMAScript modules. See [#345](https:
...
- ECMAScript module imports (i.e., `import helmet from "helmet"` and `import { frameguard } from "helmet"`). See [#320](https:
...
- `helmet.frameguard`: add TypeScript editor autocomplete. See [#322](https:
...
- Explicitly define TypeScript types in `package.json`. See [#303](https:
...
- `helmet.contentSecurityPolicy`: broken TypeScript types. See [#283](https:
...
- `helmet.expectCt`: `max-age` is now first. See [#264](https:
...
- `helmet.contentSecurityPolicy`:
  - Directive values can now include functions, as they could in Helmet 3. See [#243](https:
...
- The `HelmetOptions` interface is no longer exported. This only affects TypeScript users. If you need the functionality back, see [this comment](https:

## 4.0.0 - 2020-08-02

See the [Helmet 4 upgrade guide](https:
...
- `helmet.contentSecurityPolicy`:
  ...
  - Duplicate keys now throw an error. See [helmetjs/csp#73](https:
  - This middleware is more lenient, allowing more directive names or values
- `helmet.xssFilter` now disables the buggy XSS filter by default. See [#230](https:
...
- `helmet.contentSecurityPolicy`:
  - Removed browser sniffing (including the `browserSniff` and `disableAndroid` parameters). See [helmetjs/csp#97](https:
  - Removed conditional support. This includes directive functions and support for a function as the `reportOnly`. [Read this if you need help.](https:
  - Removed a lot of checks—you should be checking your CSP with a different tool
  - Removed support for legacy headers (and therefore the `setAllHeaders` parameter). [Read this if you need help.](https:
  - Removed the `loose` option
  - Removed support for functions as directive values. You must supply an iterable of strings
- `helmet.frameguard`:
  - Dropped support for the `ALLOW-FROM` action. [Read more here.](https:
- `helmet.hidePoweredBy` no longer accepts arguments. See [this article](https:
- `helmet.hsts`:
  - Dropped support for `includeSubdomains` with a lowercase D. See [#231](https:
  - Dropped support for `setIf`. [Read this if you need help.](https:
- `helmet.xssFilter` no longer accepts options. Read ["How to disable blocking with X-XSS-Protection"](https:
...
- Updated `helmet-csp` to v2.10.0
  - Add support for the `allow-downloads` sandbox directive. See [helmet-csp#103](https:

### Deprecated

- `helmet.noCache` is deprecated. Use the `nocache` module instead. See [#215](https:
...
- Updated `helmet-csp` to v2.9.5
  ...
  - Fixed an issue some people were having when importing the `bowser` subdependency. See [helmet-csp#96](https:
...
- Updated `helmet-csp` to v2.9.4
  - Updated `bowser` subdependency from 2.6.1 to 2.7.0. See [helmet-csp#94](https:
...
- Updated `helmet-csp` to v2.9.1
  - Updated `bowser` subdependency from 2.5.3 to 2.5.4. See [helmet-csp#88](https:
...
- Changelog is now in the [Keep A Changelog](https:
- Dropped support for Node <4. See [the commit](https:
- Updated Adam Baldwin's contact information

### Deprecated

- `helmet.hsts`'s `setIf` option has been deprecated and will be removed in `hsts@3`. See [helmetjs/hsts#22](https:

* The `includeSubdomains` option (with a lowercase `d`) has been deprecated and will be removed in `hsts@3`. Use the uppercase-D `includeSubDomains` option instead. See [helmetjs/hsts#21](https:

## 3.15.1 - 2019-02-10

### Deprecated

- The `hpkp` middleware has been deprecated. If you still need to use this module, install the standalone `hpkp` module from npm. See [#180](https:
